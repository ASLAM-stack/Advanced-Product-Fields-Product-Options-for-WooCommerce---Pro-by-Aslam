<?php
/**
 * Frontend Template: Time Picker Field (Pro).
 *
 * @package APF_Aslam
 */
...
<div class="apf-field-wrap apf-type-time" id="apf-wrap-<?php echo esc_attr( $id ); ?>" data-field-id="<?php echo esc_attr( $id ); ?>" data-field-type="time" data-pricing-type="<?php echo esc_attr( $p_type ); ?>" data-pricing-amount="<?php echo esc_attr( $p_amount ); ?>" data-conditions="<?php echo esc_attr( wp_json_encode( $field['conditional_logic'] ?? array() ) ); ?>" <?php echo $required ? 'data-required="yes"' : ''; ?>>
	...
	<div class="apf-input-wrapper apf-icon-input-wrap">
		<input 
			...
			class="apf-input apf-time-input"
			value="<?php echo esc_attr( $field['default_value'] ?? '' ); ?>"
			<?php echo $required ? 'data-required="yes"' : ''; ?>
		>
		<span class="apf-input-icon"><i class="fa-regular fa-clock"></i></span>
	</div>
